...
import { isWebMode, isDesktopMode } from './src/utils/env-check.js';
...
const APP_MODE = isWebMode() ? 'web' : 'desktop';
console.log('AI Quiz Maker running in', APP_MODE, 'mode');
...
function _rateKey() { return APP_MODE === 'web' ? `rate_ts_${authUser?.sub || 'anon'}` : `rate_ts_anon`; }
...
function getMaxQuestions() {
  ...
  const providerId = settings.activeProvider || 'default';
  return providerId === 'default' ? 30 : 50;
}

function updateQuestionLimit() {
  const max = getMaxQuestions();
  el.jumlahSoal.max = max;
  const val = parseInt(el.jumlahSoal.value);
  if (val > max) el.jumlahSoal.value = max;
...
async function handleFileSelect(file) {
  ...
  if (!result.success) {
    showStatus(result.error || 'Failed to process file.', 'error');
  ...
  // Hide loading status area immediately after completion
...
function setupSettingsUI() {
  let selectedProviderId = 'default';
  let isKeyVisible = false;

  el.settingsBtn.addEventListener('click', () => {
    const settings = loadSettings();
    selectedProviderId = settings.activeProvider || 'default';
  ...
  const closeSettings = () => {
    ...
    el.settingsTestResult.classList.add('hidden');
  };
  el.settingsClose.addEventListener('click', closeSettings);
  el.settingsCancel.addEventListener('click', closeSettings);

  el.settingsToggleKeyVisibility.addEventListener('click', () => {
    isKeyVisible = !isKeyVisible;
    el.settingsApiKey.type = isKeyVisible ? 'text' : 'password';
    el.settingsToggleKeyVisibility.innerHTML = isKeyVisible ? '<i class="fas fa-eye-slash"></i>' : '<i class="fas fa-eye"></i>';
  });

  el.settingsTestConnection.addEventListener('click', async () => {
    const config = gatherConfig(selectedProviderId);
    el.settingsTestResult.className = 'mt-2 text-sm';
    el.settingsTestResult.classList.remove('hidden');
    el.settingsTestResult.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Testing...';
    const result = await testProviderConnection(selectedProviderId, config);
    el.settingsTestResult.innerHTML = result.success
      ? `<span class="text-green-600"><i class="fas fa-check-circle"></i> ${result.message}</span>`
      : `<span class="text-red-600"><i class="fas fa-exclamation-circle"></i> ${result.message}</span>`;
  });

  el.settingsSave.addEventListener('click', () => {
    const newSettings = {
      activeProvider: selectedProviderId,
      customProviders: loadSettings().customProviders || {},
      localEndpoint: '',
      localModel: '',
    ...
    if (selectedProviderId === 'local') {
      newSettings.localEndpoint = config.endpoint || '';
      newSettings.localModel = config.model || '';
    } else if (selectedProviderId !== 'default') {
      newSettings.customProviders[selectedProviderId] = { apiKey: config.apiKey || '', model: config.model || '', endpoint: config.endpoint || '' };
    }
    ...
    showToast(`Active AI: ${activeCfg.config.description}`, 'success');
  });
  ...
  function renderProviderCards(activeProviderId) {
    ...
    PROVIDER_DEFINITIONS.forEach(p => {
      ...
      card.addEventListener('click', () => {
        ...
        populateConfigFields(p.id, currentSettings);
        el.settingsTestResult.classList.add('hidden');
        el.settingsTestResult.innerHTML = '';
      });
  ...
  function populateConfigFields(providerId, settings) {
    ...
    if (def.requiresKey) {
      el.settingsKeyField.classList.remove('hidden');
      el.settingsKeyLabel.textContent = def.keyLabel || 'API Key';
      el.settingsApiKey.placeholder = def.keyPlaceholder || 'Enter API key...';
      const customData = (settings.customProviders && settings.customProviders[providerId]) || {};
      el.settingsApiKey.value = customData.apiKey || '';
    } else {
      el.settingsKeyField.classList.add('hidden');
      el.settingsApiKey.value = '';
    }

    if (def.hideModel) {
      el.settingsModelField.classList.add('hidden');
    } else if (providerId === 'local') {
      el.settingsModelField.classList.remove('hidden');
      el.settingsModel.classList.add('hidden');
      el.settingsCustomModel.classList.remove('hidden');
      el.settingsCustomModel.placeholder = 'Enter model name';
      el.settingsCustomModel.value = settings.localModel || '';
      const fetchBtn = el.settingsLocalFetchModels;
      const fetchMsg = el.settingsLocalFetchMessage;
      if (fetchBtn) fetchBtn.classList.remove('hidden');
      if (fetchMsg) fetchMsg.classList.add('hidden');
      fetchBtn.onclick = null;
      fetchBtn.onclick = async () => {
        const endpoint = el.settingsEndpoint.value.trim() || 'http://localhost:1234/v1';
        ...
        try {
          const res = await fetch(`${baseEndpoint}/models`, { signal: AbortSignal.timeout(5000) });
          ...
          el.settingsModel.classList.remove('hidden');
          el.settingsCustomModel.classList.add('hidden');
          el.settingsModel.innerHTML = '<option value="">Select a model...</option>';
          models.forEach(m => { const opt = document.createElement('option'); opt.value = m; opt.textContent = m; el.settingsModel.appendChild(opt); });
          const customOpt = document.createElement('option'); customOpt.value = '__custom__'; customOpt.textContent = 'Custom (Type your own)...'; el.settingsModel.insertBefore(customOpt, el.settingsModel.firstChild);
          if (fetchMsg) { fetchMsg.innerHTML = `<span class="text-green-600"><i class="fas fa-check"></i> ${models.length} model(s) detected</span>`; setTimeout(() => fetchMsg.classList.add('hidden'), 3000); }
        } catch (e) {
          el.settingsModel.classList.add('hidden');
          el.settingsCustomModel.classList.remove('hidden');
          if (fetchMsg) { fetchMsg.innerHTML = '<span class="text-yellow-600"><i class="fas fa-exclamation-triangle"></i> Could not auto-detect. Please enter manually or check server.</span>'; setTimeout(() => fetchMsg.classList.add('hidden'), 4000); }
        }
      };
    } else {
      el.settingsModelField.classList.remove('hidden');
      el.settingsModel.classList.remove('hidden');
      const options = PROVIDER_MODEL_OPTIONS[def.modelOptions] || [];
      const currentModel = (settings.customProviders && settings.customProviders[providerId])?.model || def.defaultModel || '';
      el.settingsModel.innerHTML = '<option value="">Select a model...</option>';
      ...
      options.forEach(opt => {
        ...
        if (opt.value === currentModel) { option.selected = true; matchedPreset = true; }
        el.settingsModel.appendChild(option);
      });
      ...
      el.settingsModel.onchange = () => {
        if (el.settingsModel.value === '__custom__') { el.settingsCustomModel.classList.remove('hidden'); el.settingsCustomModel.focus(); }
        else { el.settingsCustomModel.classList.add('hidden'); el.settingsCustomModel.value = ''; }
      };
    }

    if (def.requiresEndpoint) {
      el.settingsEndpointField.classList.remove('hidden');
      el.settingsEndpoint.placeholder = def.defaultEndpoint || 'http://localhost:1234/v1';
      el.settingsEndpoint.value = providerId === 'local' ? (settings.localEndpoint || def.defaultEndpoint || '') : '';
    } else if (def.id !== 'default' && def.defaultEndpoint) {
      el.settingsEndpointField.classList.remove('hidden');
      el.settingsEndpoint.placeholder = def.defaultEndpoint;
      const customData = (settings.customProviders && settings.customProviders[providerId]) || {};
      el.settingsEndpoint.value = customData.endpoint || def.defaultEndpoint || '';
    } else {
      el.settingsEndpointField.classList.add('hidden');
    }
  }
  ...
  function gatherConfig(providerId) {
    ...
    let model = '';
    if (providerId === 'local') {
      if (!el.settingsModel.classList.contains('hidden')) model = el.settingsModel.value === '__custom__' ? el.settingsCustomModel.value.trim() : el.settingsModel.value;
      else model = el.settingsCustomModel.value.trim();
    } else if (def && def.hideModel) { model = ''; }
    ...
    if (providerId === 'default') { config.apiKey = import.meta.env.VITE_GEMINI_API_KEY || ''; config.endpoint = 'https://generativelanguage.googleapis.com/v1beta'; }
    if (!config.endpoint) {
      if (def && def.defaultEndpoint) config.endpoint = def.defaultEndpoint;
      else if (providerId === 'gemini') config.endpoint = 'https://generativelanguage.googleapis.com/v1beta';
      else if (providerId === 'anthropic') config.endpoint = 'https://api.anthropic.com/v1';
      else if (providerId === 'openai') config.endpoint = 'https://api.openai.com/v1';
    }
    return config;
  }
}
...
function showLoading(show) {
  ...
  if (show) {
    ...
    _loadingInterval = setInterval(() => { const target = 90; if (_loadingProgress < target) { const step = _loadingProgress < 40 ? 3 : _loadingProgress < 70 ? 2 : 1; setLoadingProgress(_loadingProgress + step); } }, 400);
  } else { clearInterval(_loadingInterval); setLoadingProgress(100); setTimeout(() => setLoadingProgress(0), 600); }
}

// Clean up loading states everywhere
function clearAllLoadingStates() {
  el.statusArea.classList.add('hidden');
  if (el.loadingOverlay) showLoading(false);
  el.generateBtn.disabled = false;
  el.generateBtn.innerHTML = '<i class="fas fa-sparkles"></i> Generate';
}
...
async function handleGenerateQuiz() {
  ...
  if (jumlahSoal > max) return showToast('Max ' + max + ' questions for current provider.', 'error');
  if (activeTab === 'text') {
    materi = el.materi.value.trim();
    if (materi.length < 50) return showToast('Minimum 50 characters.', 'error');
    if (containsAdultContent(materi)) return showToast('18+ content not allowed.', 'error');
  } else if (!file) return showToast('Please select a file.', 'error');
  disableBtn(true);
  if (el.loadingOverlay) showLoading(true);
  setLoadingProgress(10);
  showStatus('Generating quiz...', 'loading');
  showSkeleton();
  try {
    if (activeTab === 'file' && file) {
      setLoadingProgress(25);
      if (window._fileMarkdown) { materi = window._fileMarkdown.trim(); }
      else {
        const processed = await processDocument(file);
        if (!processed.success) throw new Error(processed.error || 'Failed to extract text.');
        materi = processed.markdown;
      }
      if (!materi || materi.length < 50) throw new Error('Text too short.');
    ...
    const result = await aiGenerateQuiz(materi, { difficulty, questionCount: jumlahSoal });
    ...
    if (questions.length < jumlahSoal) showToast(`Quiz completed (${questions.length}/${jumlahSoal} questions).`, 'warning');
    else showToast('Quiz ready!', 'success');
...
function showToast(msg, type = 'info') {
  ...
  const colors = { success: 'bg-green-100 border-green-500 text-green-800', error: 'bg-red-100 border-red-500 text-red-800', warning: 'bg-yellow-100 border-yellow-500 text-yellow-800', info: 'bg-blue-100 border-blue-500 text-blue-800' };
  toast.className = (colors[type] || colors.info) + ' pointer-events-auto px-4 py-3 rounded-lg border-l-4 shadow-lg text-sm max-w-sm';
  toast.innerHTML = msg;
...
function setupExportModal() {
  const modal = document.getElementById('exportModal');
  if (!modal) return;
  document.getElementById('exportModalClose').addEventListener('click', () => modal.classList.add('hidden'));
  document.getElementById('exportModalCancel').addEventListener('click', () => modal.classList.add('hidden'));
  document.getElementById('exportSummary').addEventListener('click', () => {
    ...
    a.download = `quiz-summary-${new Date().toISOString().slice(0,10)}.txt`;
    a.click();
  ...
  document.getElementById('exportSeparated').addEventListener('click', async () => {
    ...
    try {
      ...
      if (!JSZip) { showToast('ZIP library not loaded. Try Summary instead.', 'error'); return; }
      const zip = new JSZip();
      const questionsText = currentQuiz.map((q, i) => `${i + 1}. ${q.question}\n${q.answerOptions.map(o => `  ${o.isCorrect ? '○' : '○'} ${o.text}`).join('\n')}`).join('\n\n');
      ...
      zip.file('questions.txt', questionsText);
      zip.file('answer-key.txt', answersText);
      const blob = await zip.generateAsync({ type: 'blob' });
      const a = document.createElement('a');
      a.href = URL.createObjectURL(blob);
      a.download = `quiz-separated-${new Date().toISOString().slice(0,10)}.zip`;
      a.click();
      URL.revokeObjectURL(a.href);
      showToast('ZIP with separated files downloaded!', 'success');
...
function showStatus(msg, type) {
  ...
  area.className = 'p-4 rounded-lg text-center transition-all';
  if (type === 'loading') area.className += ' bg-blue-100 text-blue-700';
  else { area.className += ' bg-green-100 text-green-700'; }
  area.classList.remove('hidden');
  if (type !== 'loading') {
    setTimeout(() => {
      el.statusArea.classList.add('hidden');
      el.statusArea.className = 'p-4 rounded-lg text-center transition-all hidden';
    }, 5000);
  }
}
...
function handleReset() {
  ...
  el.fileInput.value = '';
  selectedFile = null;
  window._fileMarkdown = null;
  if (el.fileCard) el.fileCard.classList.add('hidden');
  el.fileName.textContent = '';
  if (el.clearFileButton) el.clearFileButton.classList.add('hidden');
  if (el.clearAllFilesButton) el.clearAllFilesButton.classList.add('hidden');
...
function setupRegenerateModal() {
  ...
  const setActive = (buttons, keys, current) => {
    const activeCls = ['bg-blue-600','dark:bg-indigo-500','text-white'];
    const inactiveCls = ['bg-gray-200','dark:bg-slate-700','text-gray-800','dark:text-gray-100'];
    buttons.forEach((b, idx) => { if (!b) return; b.classList.remove(...activeCls, ...inactiveCls); const isActive = keys[idx] === current; b.classList.add('px-3','py-2','rounded-full','border','border-gray-300','dark:border-gray-600', ...(isActive ? activeCls : inactiveCls)); });
  };
  ...
  const clampRegenCount = () => {
    ...
    if (isNaN(val)) val = 1;
    if (val < 1) val = 1;
    if (val > max) { val = max; if (regenWarning) { regenWarning.classList.remove('hidden'); document.getElementById('regenWarningMax').textContent = max; setTimeout(() => { if (regenWarning) regenWarning.classList.add('hidden'); }, 3000); } }
...
function openRegenModal(e) {
  if (e) { e.preventDefault(); e.stopImmediatePropagation(); }
  if (el.regenModal) el.regenModal.classList.remove('hidden');
  if (el.regenJumlahSoal) {
    el.regenJumlahSoal.value = currentQuiz.length > 0 ? currentQuiz.length : (lastState.jumlahSoal || 5);
  }
}
...
async function handleRegenerate(opts = {}) {
  ...
  const max = getMaxQuestions();
  if (!lastState.materiText || !optJumlah) return showToast('No context to regenerate.', 'error');
  const jumlahSoal = Math.min(optJumlah, max);
  if (optJumlah > max) showToast('Limited to max ' + max + ' questions.', 'warning');
  if (containsAdultContent(lastState.materiText)) return showToast('18+ content not allowed.', 'error');
  disableBtn(true);
  if (el.loadingOverlay) showLoading(true);
  ...
  try {
    if (!canConsumeRateLimit()) throw new Error('Too frequent: max 10 requests per 3 minutes.');
    setLoadingProgress(40);
    const result = await aiGenerateQuiz(lastState.materiText, { difficulty: optDiff, questionCount: jumlahSoal });
    if (!result.success) throw new Error(result.error);
    setLoadingProgress(80);
    const questions = result.quiz.questions;
    currentQuiz = questions;
    displayQuiz(questions);
    setLoadingProgress(95);
    if (questions.length < jumlahSoal) showToast(`New quiz (${questions.length}/${jumlahSoal} questions).`, 'warning');
    else showToast('New quiz ready!', 'success');
  } catch (err) { showToast(`Error: ${err.message}`, 'error'); }
  finally { clearAllLoadingStates(); }
}
...
function setupQuickButtons() {
  document.querySelectorAll('.quick-btn[data-target="jumlahSoal"]').forEach(btn => {
    btn.addEventListener('click', () => { if (el.jumlahSoal) { el.jumlahSoal.value = parseInt(btn.dataset.count); updateQuestionLimit(); const hw = document.getElementById('homeWarning'); if (hw) hw.classList.add('hidden'); } });
  });
}
...
function setupHomeJumlahWarning() {
  ...
  const clamp = () => {
    ...
    if (isNaN(val)) val = 1;
    if (val < 1) val = 1;
    if (val > max) { val = max; hw.classList.remove('hidden'); setTimeout(() => hw.classList.add('hidden'), 3000); } else hw.classList.add('hidden');
    el.jumlahSoal.value = val;
  };
  el.jumlahSoal.addEventListener('input', clamp);
  el.jumlahSoal.addEventListener('change', clamp);
...
function setupTutorial() {
  ...
  window._tutorialShowStep = showStep;
...
if (APP_MODE === 'web') {
  authToken = localStorage.getItem('google_id_token') || null;
  authUser = JSON.parse(localStorage.getItem('google_user') || 'null');
  sessionTs = parseInt(localStorage.getItem('google_session_ts') || '0');
  ...
  function setupAuth() {
    const renderButtons = (clientId) => {
      if (window.google) {
        ...
        const btnModal = document.getElementById('googleSignInBtnModal');
        if (btn) google.accounts.id.renderButton(btn, { theme: 'filled_blue', size: 'medium', shape: 'rectangular', text: 'signin_with' });
        if (btnModal) google.accounts.id.renderButton(btnModal, { theme: 'filled_blue', size: 'medium', shape: 'rectangular', text: 'signin_with' });
    ...
    const updateAuthUI = () => {
      ...
      if (valid && authUser) { if (authArea) authArea.classList.remove('hidden'); updateAuthDisplay(); const logoutBtn = document.getElementById('logoutBtn'); if (logoutBtn) { logoutBtn.classList.remove('hidden'); logoutBtn.onclick = handleLogout; } const googleBtn = document.getElementById('googleSignInBtn'); if (googleBtn) googleBtn.classList.add('hidden'); showAuthOverlay(false); }
      else { if (authArea) authArea.classList.add('hidden'); hideAuthDisplay(); const logoutBtn = document.getElementById('logoutBtn'); if (logoutBtn) logoutBtn.classList.add('hidden'); const googleBtn = document.getElementById('googleSignInBtn'); if (googleBtn) googleBtn.classList.remove('hidden'); showAuthOverlay(true); }
    };
    const clientId = import.meta.env.VITE_GOOGLE_CLIENT_ID;
    if (clientId && clientId !== 'YOUR_GOOGLE_CLIENT_ID') { renderButtons(clientId); }
    else { showAuthOverlay(true); showToast('Google Client ID not configured.', 'warning'); }
  }
  ...
  function onGoogleSignIn(response) {
    ...
    let user = null;
    try { const payload = JSON.parse(atob(parts[1].replace(/-/g, '+').replace(/_/g, '/'))); user = { email: payload.email, name: payload.name, sub: payload.sub, picture: payload.picture || null }; } catch {}
  ...
  function handleLogout() {
    ...
    const authArea = document.getElementById('authArea'); if (authArea) authArea.classList.add('hidden');
    hideAuthDisplay();
    const logoutBtn = document.getElementById('logoutBtn'); if (logoutBtn) logoutBtn.classList.add('hidden');
    showAuthOverlay(true);
    showToast('You have logged out.', 'info');
  }

  function isSessionValid() { return !!authToken && !!sessionTs && (Date.now() - sessionTs < SESSION_MAX_AGE_MS); }

  function showAuthOverlay(show) {
    const overlay = document.getElementById('authOverlay'); if (!overlay) return;
  ...
  function updateAuthDisplay() {
    ...
    if (authUser) {
      ...
      if (authUser.picture) { avatar.src = authUser.picture; avatar.classList.remove('hidden'); fallback.classList.add('hidden'); avatar.onerror = () => { avatar.classList.add('hidden'); fallback.classList.remove('hidden'); fallback.textContent = (authUser.name || 'U').charAt(0).toUpperCase(); }; }
      else { avatar.classList.add('hidden'); fallback.classList.remove('hidden'); fallback.textContent = (authUser.name || 'U').charAt(0).toUpperCase(); }
    }
  }

  function hideAuthDisplay() { const profile = document.getElementById('userProfile'); if (profile) profile.classList.add('hidden'); }

  function setupAuthRetry() {
    const retryBtn = document.getElementById('retryLoginButton'); if (!retryBtn) return;
    retryBtn.addEventListener('click', () => {
      ...
      s.onerror = () => showToast('Failed to load Google Sign-In script.', 'error');
      document.head.appendChild(s);
      setTimeout(() => { try { setupAuth(); } catch {} }, 1000);
    });
  }

  const _origHandleGenerateQuiz = handleGenerateQuiz;
  handleGenerateQuiz = async function() { if (!isSessionValid()) { showAuthOverlay(true); return showToast('Please sign in.', 'error'); } return _origHandleGenerateQuiz.apply(this, arguments); };
  const _origHandleRegenerate = handleRegenerate;
  handleRegenerate = async function(opts = {}) { if (!isSessionValid()) { showAuthOverlay(true); return showToast('Please sign in.', 'error'); } return _origHandleRegenerate.apply(this, opts); };

  if (document.readyState === 'loading') { document.addEventListener('DOMContentLoaded', () => { setTimeout(() => { setupAuth(); setupAuthRetry(); }, 100); }); }
  else { setTimeout(() => { setupAuth(); setupAuthRetry(); }, 100); }
}
